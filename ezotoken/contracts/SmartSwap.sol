...
contract SmartSwap is SafeMath, Haltable {
    ...
    function setCurrencyPricesContractAddress(address _currencyPricesContract)
        public
        onlyOwner
    {
    ...
    function setSecureETHContractAddress(address _secureETHContractAddress)
        public
        onlyOwner
    {
    ...
    function getPendingOrdersTest(
        address currencySent,
        address currencyWant,
        uint256 index
    ) public view returns (uint256, address, address, bool) {
        return (
            orderAll[currencySent][currencyWant].orderDetails[index]
                .remainingAmount,
            orderAll[currencySent][currencyWant].orderDetails[index]
                .currencySent,
            orderAll[currencySent][currencyWant].orderDetails[index]
                .currencyWant,
            orderAll[currencySent][currencyWant].orderDetails[index].status
        );
    }

    function sendEther(address _currencyWant) external payable {
        addOrder(msg.value, msg.sender, address(0), _currencyWant);
    }

    function sendToken(address token, uint256 amount, address _currencyWant)
        public
    {
    ...
    function cancelOrder(
        ...
        address _currencyWant,
        uint256 _index
    ) public {
        require(
            msg.sender ==
                orderAll[_currencySent][_currencyWant].orderDetails[_index]
                    .sender
        );
        require(
            !orderAll[_currencySent][_currencyWant].orderDetails[_index].status
        );
        orderAll[_currencySent][_currencyWant].orderDetails[_index]
            .status = true;
        generalFundAssign(
            _currencySent,
            orderAll[_currencySent][_currencyWant].orderDetails[_index].sender,
            orderAll[_currencySent][_currencyWant].orderDetails[_index]
                .remainingAmount
        );
    }
    ...
    function addOrder(
        ...
        if (_currencyWant == ezoTokenAddress) {
            orderAll[_currencySent][_currencyWant].orderDetails.push(
                PurchaseRecord(
                    _sender,
                    _sentAmount,
                    0,
            ...
            tokenBalancesForEZO[_currencySent] = safeAdd(
                tokenBalancesForEZO[_currencySent],
                _sentAmount
            );
            generalFundAssign(
                ezoTokenAddress,
                            ...
                            safeMul(
                                ...
                                10 **
                                    CurrencyPrices(currencyPricesContract)
                                        .currencyDecimal(ezoTokenAddress)
                            ),
                            10 **
                                CurrencyPrices(currencyPricesContract)
                                    .currencyDecimal(_currencySent)
                        ),
                ...
                generalFundAssignEZO(
                    ...
                    safeDiv(
                        safeMul(
                            ...
                            CurrencyPrices(currencyPricesContract)
                                .currencyPrices(ezoTokenAddress)
                        ),
                        CurrencyPrices(currencyPricesContract).currencyPrices(
                            _currencyWant
                        )
        ...
        emit orderAdded(
            (orderAll[_currencySent][_currencyWant].orderDetails.length - 1),
            _sentAmount,
            _sender,
            _currencySent,
            _currencyWant
    ...
    function checkStability(
        address _currencySentNewOrder,
        address _currencyWantNewOrder,
        uint256 _indexNew,
        uint256 _indexOld
    ) public returns (bool) {
        require(!inExecution);
        require(
            CurrencyPrices(currencyPricesContract).currencyPrices(
                _currencySentNewOrder
            ) >
                0 &&
                CurrencyPrices(currencyPricesContract).currencyPrices(
                    _currencyWantNewOrder
                ) >
                0
        );
        inExecution = true;
        address currencySentNewOrder = orderAll[_currencySentNewOrder][_currencyWantNewOrder]
            .orderDetails[_indexNew]
            .currencySent;
        address currencyWantNewOrder = orderAll[_currencySentNewOrder][_currencyWantNewOrder]
            .orderDetails[_indexNew]
            .currencyWant;
        uint256 remainingAmountNewOrder = orderAll[_currencySentNewOrder][_currencyWantNewOrder]
            .orderDetails[_indexNew]
            .remainingAmount;

        if (
            _indexNew <
            orderAll[currencySentNewOrder][currencyWantNewOrder]
                .orderDetails
                .length &&
            _indexOld <
            orderAll[currencyWantNewOrder][currencySentNewOrder]
                .orderDetails
                .length
        ) {
            uint256 remainingAmountPendingOrder = orderAll[_currencyWantNewOrder][_currencySentNewOrder]
                .orderDetails[_indexOld]
                .remainingAmount;
            uint256 returnCurrencyAmount = getReturnAmount(
                remainingAmountPendingOrder,
                _currencySentNewOrder,
                _currencyWantNewOrder,
                _indexOld
            );
            ...
            if (
                !orderAll[_currencyWantNewOrder][_currencySentNewOrder]
                    .orderDetails[_indexOld]
                    .status &&
                !orderAll[_currencySentNewOrder][_currencyWantNewOrder]
                    .orderDetails[_indexNew]
                    .status
            ) {
                if (returnCurrencyAmount < remainingAmountNewOrder) {
                    ...
                    orderAll[_currencySentNewOrder][_currencyWantNewOrder]
                        .orderDetails[_indexNew]
                        .remainingAmount = safeSub(
                        remainingAmountNewOrder,
                        returnCurrencyAmount
                    );
                    generalFundAssign(
                        currencyWantNewOrder,
                        orderAll[_currencySentNewOrder][_currencyWantNewOrder]
                            .orderDetails[_indexNew]
                            .sender,
                        remainingAmountPendingOrder
                    ...
                    generalFundAssign(
                        ...
                        orderAll[_currencyWantNewOrder][_currencySentNewOrder]
                            ...
                            .sender,
                        remainingAmountNewOrder -
                            (remainingAmountNewOrder - returnCurrencyAmount)
                    );
                    inExecution = false;
    ...
    function getReturnAmount(
        uint256 _remainingAmountPendingOrder,
        address _currencySentNewOrder,
        address _currencyWantNewOrder,
        uint256 _indexOld
    ...
    function getCalValue(
        uint256 returnCurrencyAmount,
        uint256 remainingAmountNewOrder,
        address _currencySent,
        address _currencyWant
    ) internal view returns (uint256) {
        return
            ((returnCurrencyAmount - remainingAmountNewOrder) *
                (
                    safeDiv(
                        safeMul(
                            CurrencyPrices(currencyPricesContract)
                                .currencyPrices(_currencySent),
                            1 ether
                        ),
                        CurrencyPrices(currencyPricesContract).currencyPrices(
                            _currencyWant
    ...
    function generalFundAssign(
        ...
        if (_currencyWantNewOrder == ezoTokenAddress) {
            ...
            require(
                Token(_currencyWantNewOrder).balanceOf(address(this)) >= _amount
            );
    ...
    function generalFundAssignEZO(
        ...
        if (_currencyWantNewOrder == address(0)) {
            if (address(this).balance >= _amount) {
                tokenBalancesForEZO[_currencyWantNewOrder] = safeSub(
                    tokenBalancesForEZO[_currencyWantNewOrder],
                    _amount
                );
                assignEther(_recipient, _amount);
                emit orderCompleted(_amount, _recipient, address(0));
    ...
    function systemAssignToken(address token, address _to, uint256 _amount)
        internal
    {
    ...
    function _sendEther() public payable {
        ...
        tokenBalancesForEZO[address(0)] = safeAdd(
            tokenBalancesForEZO[address(0)],
            msg.value
        );
        Token(secureETHContractAddress).mint(
            msg.sender,
            safeMul(
                CurrencyPrices(currencyPricesContract).currencyPrices(
                    address(0)
    ...
    function _withdrawEther() public {
        uint256 tokensToWithdraw = Token(secureETHContractAddress).balanceOf(
            msg.sender
        );
        ...
        if (
            ...
            Token(secureETHContractAddress).burn(msg.sender, tokensToWithdraw);
            tokenBalancesForEZO[address(0)] = safeSub(
                tokenBalancesForEZO[address(0)],
                safeDiv(
                    tokensToWithdraw,
                    CurrencyPrices(currencyPricesContract).currencyPrices(
            ...
            Token(secureETHContractAddress).burn(
                msg.sender,
                safeMul(
                    safeSub(
                        address(this).balance,
            ...
            uint256 etherToSend = safeSub(
                address(this).balance,
                tokenBalancesForEZO[address(0)]
            );
            tokenBalancesForEZO[address(0)] = safeSub(
                tokenBalancesForEZO[address(0)],
                safeSub(address(this).balance, tokenBalancesForEZO[address(0)])
            );
